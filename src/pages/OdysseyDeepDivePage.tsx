...
const spacer: CSSProperties = { marginTop: '1.5rem' };
    ...
    <div style={pageWrapper}>
      ...
      <nav style={sidebarStyle}>
        <div style={{ padding: '0 20px', marginBottom: '1rem' }}>
          <div style={{ fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: c.accent }}>
            Odyssey Deep Dive
          </div>
        </div>
        {tocItems.map((item) => (
          <a
            ...
            style={activeToc === item.id ? tocLinkActiveStyle : tocLinkStyle}
          >
            {item.label}
          </a>
        ))}
        ...
        <section style={heroSection}>
          ...
          <motion.div style={heroContent} {...fadeUp}>
            ...
            <p style={heroSubtitle}>
              A comprehensive guide for JavaScript developers joining the Kotlin/Spring Boot reactive backend
            </p>
            <div style={statsRow}>
              {[
                { val: '5', label: 'Services' },
                { val: '9', label: 'Modules' },
                { val: '13', label: 'Diagrams' },
        ...
        <section id="eds-import" style={sectionSpacing}>
          ...
          <motion.div {...fadeUp}>
            <p style={para}>
              ...
              7-stage process designed for safety and consistency.
            </p>
            <CodeBlock language="text" code={`Stage 1: Find & Validate File
   Azure Blob Storage (container: inbound, prefix: CVP/Driveway-Merchandising-V2/)
      -> Find newest GZIP TSV file -> Validate -> Download

Stage 2: Parse & Stage to Temp Collection
   TSV File -> CsvToExtractedRowConverter -> EdsExtractedInventoryRow
      -> Convert to Vehicle objects -> Batch insert to temp_vehicleV3_<timestamp>
        (500 vehicles/batch, 5 concurrent semaphores)

Stage 3: Safety Check
   If (liveCount - incomingCount) >= 15,000 -> ABORT

Stage 4: Compute & Publish Deltas
   ADD = VIN in incoming, NOT in live
   UPDATE = VIN in both, price OR mileage changed
   DELETE = VIN in live, NOT in incoming
      -> Publish to inventory-delta-topic (batches of 100)

Stage 5: Merge to Live Collection
   Temp data wins for EDS fields
   PRESERVE from live: leasing, purchasePending, manuallySuppressed

Stage 6: Mark & Sweep
   Vehicles where lastSeenJobId != current -> status = DELETED

Stage 7: Apply Lease Expiries
   Filter expired regionalPrices -> Update canLease & defaultPrice`} />
          </motion.div>

          <motion.div {...fadeUp} style={spacer}>
            <MermaidViewer title="EDS Import Sequence" tabs={[{ label: 'Import Sequence', source: mermaidEdsSequence }]} />
          </motion.div>
  ...
  regionalPrices = listOf(
    RegionData(3, 31500, "2025-06-30")  // Active
        ...
        <section id="data-flows" style={sectionSpacing}>
          <SectionHeader label="Sequences" title="Additional Data Flow Diagrams" description="Leasing, cart, and search processing sequences" />

          <motion.div {...fadeUp}>
            <MermaidViewer title="Leasing Message Processing Sequence" tabs={[{ label: 'Leasing Sequence', source: mermaidLeasingSequence }]} />
          </motion.div>

          <motion.div {...fadeUp} style={spacer}>
            <MermaidViewer title="Cart Status Update Sequence" tabs={[{ label: 'Cart Sequence', source: mermaidCartSequence }]} />
          </motion.div>
